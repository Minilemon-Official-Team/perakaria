...
describe("CMS policy",()=>{
  ...
  it("allows owned draft submission and superadmin approval",()=>{expect(canRunWorkflow("admin_writer","submit-review","draft",true)).toBe(true);expect(canRunWorkflow("superadmin","approve","pending_review",true)).toBe(true)});
  ...
  it("rejects illegal uploads",()=>{expect(validateUpload("image/gif",100,"alt").status).toBe(415);expect(validateUpload("image/png",3_000_000,"alt").status).toBe(413);expect(validateUpload("image/png",100," ").status).toBe(400);expect(normalizeFileName("My Hero @2x.PNG")).toBe("my-hero-2x.png")});
});